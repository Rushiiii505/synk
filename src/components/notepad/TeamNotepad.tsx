...
export function TeamNotepad() {
  ...
  // Scratchpad height state (persisted)
  const [scratchpadHeight, setScratchpadHeight] = useState<number>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('synk_scratchpad_height');
      if (saved) return parseInt(saved, 10) || 110;
    }
    return 110;
  });

  // Note Size Density: 'compact' | 'normal' | 'large'
  const [noteDensity, setNoteDensity] = useState<'compact' | 'normal' | 'large'>('normal');
  ...
  useEffect(() => {
    localStorage.setItem('synk_scratchpad_height', scratchpadHeight.toString());
  }, [scratchpadHeight]);
  ...
  return (
    <div id="notepad-section" className="space-y-4 select-none">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm sm:text-base font-black tracking-tight text-slate-900 flex items-center gap-2">
                <span>Team Notepad & Shared Scratchpad</span>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Size-adjustable scratchpad and draggable resizable sticky memos with image diagrams.
          </p>
        </div>

        <div className="flex items-center gap-2">
          {/* Note Density Switcher */}
          <div className="flex items-center p-0.5 bg-white rounded-xl border border-slate-200 shadow-xs">
            <button
              onClick={() => setNoteDensity('compact')}
              className={`px-2 py-1 rounded-lg text-[11px] font-bold transition-all cursor-pointer ${
                noteDensity === 'compact' ? 'bg-slate-950 text-white shadow-xs' : 'text-slate-500 hover:text-slate-900'
              }`}
              title="Compact 4-column density"
            >
              Compact
            </button>
            <button
              onClick={() => setNoteDensity('normal')}
              className={`px-2 py-1 rounded-lg text-[11px] font-bold transition-all cursor-pointer ${
                noteDensity === 'normal' ? 'bg-slate-950 text-white shadow-xs' : 'text-slate-500 hover:text-slate-900'
              }`}
              title="Normal 3-column density"
            >
              Normal
            </button>
            <button
              onClick={() => setNoteDensity('large')}
              className={`px-2 py-1 rounded-lg text-[11px] font-bold transition-all cursor-pointer ${
                noteDensity === 'large' ? 'bg-slate-950 text-white shadow-xs' : 'text-slate-500 hover:text-slate-900'
              }`}
              title="Large 2-column spacious density"
            >
              Spacious
            </button>
          </div>
          ...
          <button
            ...
            className="flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-950 text-white font-bold text-xs hover:bg-slate-800 transition-all shadow-xs active:scale-95 shrink-0 cursor-pointer"
          >
      ...
      {/* 1. Size-Adjustable & Draggable Live Team Scratchpad */}
      <div className="rounded-2xl p-4 sm:p-5 bg-white border border-slate-200/90 shadow-xs space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2 pb-2.5 border-b border-slate-100">
          ...
          {/* Height Presets & Copy */}
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 bg-slate-100 p-0.5 rounded-xl border border-slate-200 text-[10px] font-bold text-slate-600">
              <button
                onClick={() => setScratchpadHeight(80)}
                className={`px-2 py-0.5 rounded-lg transition-colors cursor-pointer ${
                  scratchpadHeight <= 90 ? 'bg-white text-slate-900 shadow-xs font-black' : 'hover:text-slate-900'
                }`}
              >
                Small
              </button>
              <button
                onClick={() => setScratchpadHeight(160)}
                className={`px-2 py-0.5 rounded-lg transition-colors cursor-pointer ${
                  scratchpadHeight > 90 && scratchpadHeight <= 200 ? 'bg-white text-slate-900 shadow-xs font-black' : 'hover:text-slate-900'
                }`}
              >
                Medium
              </button>
              <button
                onClick={() => setScratchpadHeight(300)}
                className={`px-2 py-0.5 rounded-lg transition-colors cursor-pointer ${
                  scratchpadHeight > 200 ? 'bg-white text-slate-900 shadow-xs font-black' : 'hover:text-slate-900'
                }`}
              >
                Large (Full)
              </button>
        ...
        {/* Drag-to-Resize Scratchpad Text Area */}
        <div className="relative">
          <textarea
            style={{ height: `${scratchpadHeight}px`, minHeight: '70px', resize: 'vertical' }}
            value={scratchpadContent}
            onChange={(e) => setScratchpadContent(e.target.value)}
            placeholder="Type live notes, meeting minutes, or checklists... Drag bottom-right corner to adjust height!"
            className="w-full p-3.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-mono text-slate-800 leading-relaxed focus:outline-none focus:border-indigo-500 focus:bg-white transition-all shadow-inner block"
          />
          <div className="flex items-center justify-between text-[10px] text-slate-400 mt-1 px-1">
            ...
            <span className="flex items-center gap-1 italic">
              ...
              <span>Drag bottom edge to resize height</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {/* Sticky Notes Grid (Dynamic Density Columns) */}
            ...
            {filteredNotes.map((note) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Draggable Resizable Note Content */}
                  <textarea
                    rows={noteDensity === 'compact' ? 3 : noteDensity === 'large' ? 8 : 4}
                    style={{ resize: 'vertical', minHeight: '60px' }}
                    value={note.content}
                    onChange={(e) => updateStickyNote(note.id, { content: e.target.value })}
                    className={`w-full bg-transparent text-xs ${theme.text} leading-relaxed focus:outline-none rounded-lg p-1`}
                  />
